...
class ActionMakeTransaction(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        if 'transaction-type' in missing_slots:
            ...
            return []
        if 'transaction-type' in missing_slots:
            dispatcher.utter_message(response='Enter Acount number you want to send')
            return []
        if 'transaction-type' in missing_slots:
            dispatcher.utter_message(response='Enter IFSC code')
            return []
        ...
        # try:
        dispatcher.utter_message(text=f"Amount has been deducted sucessfully")
        # except:
...
class ActionAccountDetail(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        x = requests.post(url, data=data, headers=headers)
        #profile = json.loads(x.json())['profile']
        dispatcher.utter_message(text=f"Title : Shreyas Jadhav")
        dispatcher.utter_message(text=f"Account Number : 1501 2001 0705 2002 ")
        dispatcher.utter_message(text=f"Account Type : Savings")
        dispatcher.utter_message(text=f"Current Balance: ₹ 24,30,200")
        return []
